...
/**
 * Created by cryst on 18/03/2018.
 */
...
public class DatabaseHelper extends SQLiteOpenHelper{
    ...
    public Cursor getContactId(Contact contact){
        SQLiteDatabase db = this.getWritableDatabase();
        String sql = "SELECT * FROM "+TABLE_NAME+" WHERE "+COL1+" = '"+contact.getName()+"' AND "+COL2+" = '"+contact.getPhonenumber()+"'";

        return db.rawQuery(sql, null);
    }

    public Integer deteleContact(int id){
        SQLiteDatabase db = this.getWritableDatabase();
        return db.delete(TABLE_NAME, COL0+" = ? ", new String[]{String.valueOf(id)});
    }
}
